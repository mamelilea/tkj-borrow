-- ============================================
-- Database Schema for TKJ Peminjaman Barang (PostgreSQL)
-- Converted from MySQL to PostgreSQL for Supabase
-- ============================================

-- PostgreSQL tidak perlu CREATE DATABASE di schema
-- Supabase akan handle database creation
...
CREATE TABLE IF NOT EXISTS barang (
  ...
  jumlah_dipinjam INTEGER NOT NULL DEFAULT 0,
  foto_barang VARCHAR(255),
  notes TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_kode_barang ON barang(kode_barang);
...
CREATE TABLE IF NOT EXISTS peminjaman (
  ...
  signature TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_barang FOREIGN KEY (id_barang) REFERENCES barang(id_barang) ON DELETE RESTRICT
);

CREATE INDEX IF NOT EXISTS idx_kode_peminjaman ON peminjaman(kode_peminjaman);
CREATE INDEX IF NOT EXISTS idx_status ON peminjaman(status);
CREATE INDEX IF NOT EXISTS idx_tanggal_pinjam ON peminjaman(tanggal_pinjam);
CREATE INDEX IF NOT EXISTS idx_peminjaman_tanggal ON peminjaman(tanggal_pinjam DESC);
CREATE INDEX IF NOT EXISTS idx_peminjaman_status_tanggal ON peminjaman(status, tanggal_pinjam DESC);
...
CREATE INDEX IF NOT EXISTS idx_username ON admin(username);

-- ============================================
-- Data Awal: Admin
-- ============================================
-- Password: admin123 (hashed dengan bcrypt)
-- Note: Ganti password hash dengan yang baru jika perlu
INSERT INTO admin (username, password, nama_lengkap) VALUES 
('admin', '$2b$10$rQzJ6KkY8K8J8K8J8K8JuY8K8J8K8J8K8J8K8J8K8J8K8J8K8J8K8K', 'Administrator TKJ')
...
SELECT 
  ...
  b.nama_barang,
  b.foto_barang,
  EXTRACT(DAY FROM (COALESCE(p.tanggal_kembali, NOW()) - p.tanggal_pinjam)) AS lama_pinjam_hari
FROM peminjaman p
JOIN barang b ON p.id_barang = b.id_barang;

-- ============================================
-- Functions dan Triggers (PostgreSQL)
-- ============================================

-- Function: Auto-update jumlah_dipinjam saat peminjaman baru
CREATE OR REPLACE FUNCTION update_stok_on_insert()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION update_stok_on_insert();

-- Function: Update stok saat status peminjaman berubah
CREATE OR REPLACE FUNCTION update_stok_on_update()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION update_stok_on_update();

-- ============================================
-- Enable Row Level Security (RLS) untuk Supabase
-- ============================================
-- Note: Kita perlu izinkan semua akses untuk backend
-- Tapi bisa di-restrict untuk keamanan jika perlu

-- Untuk produksi, bisa disable RLS atau buat policy
-- Disable RLS sementara untuk backend access
